...
from forecasting_service.parsers.domclick.ssr_state import (

    extract_ssr_state_safe,

)

from forecasting_service.parsers.domclick.state_parser import (
...
class DomclickListingParser:
    ...
    def _merge_detail_dicts(
        ...
        merged = dict(html_details or {})

        for key, value in (ssr_details or {}).items():
    ...
    def collect_details(
        ...
        while processed < batch_size:
            ...
            flat = self.storage.get_next_for_detail(source="domclick")
            ...
            try:
                ...
                state = extract_ssr_state_safe(html)

                if state is not None:

                    try:

                        ssr_details = parse_detail_state(state)
                ...
                try:
                    ...
                    html_details = parse_detail_page(html)

                except Exception as e:

                    logger.debug(f" HTML fallback parse failed: {e}")

                details = self._merge_detail_dicts(
                ...
                if not details:
                    ...
                    raise ValueError("Не удалось извлечь детали ни из SSR, ни из HTML")
                ...
                logger.info(
                    ...
                    f"(SSR={'yes' if ssr_details else 'no'}, "

                    f"ssr_keys={len(ssr_details)}, html_keys={len(html_details)})"
